...
import com.velitask.sdk.properties.ImageSourceProperty;
...
public class ImageIndicator extends Indicator {
    ...
    private final ImageSourceProperty mImage = new ImageSourceProperty();

    private final BorderProperty mBorder = new BorderProperty();

    public ImageSourceProperty getImage() {
        return mImage;
    }
    ...
    public void render(IndicatorContext indicatorContext) {
        ImageContext ctx = (ImageContext) indicatorContext;
        ...
        long rawTime = mImage.convertToRawTime(ctx.player.time);
        BufferedImage frame = ctx.image.frameAt(rawTime, innerW, innerH);
        if (frame == null) {
            return;
        }
    ...
    public class ImageContext extends IndicatorContext {

        public final BorderProperty.BorderContext border;
        public final ImageSourceProperty.ImageSourceContext image;

        public ImageContext(Player player, Canvas canvas) {
            super(player, canvas);
            border = mBorder.createContext();
            image = mImage.createContext();
        }
    }

    @Override
    public IndicatorContext createContext(IndicatorContext.Player player, IndicatorContext.Canvas canvas) {
        return new ImageContext(player, canvas);
    }
}
